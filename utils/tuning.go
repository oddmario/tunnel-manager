...
func SysTuning(shouldEnableIPIPmod, shouldEnableGREmod, shouldEnableWGmod bool, mode, mainNetworkInterface string, applyTuningTweaks bool) {
	...
	if mode == "tunnel_host" {
		...
		if applyTuningTweaks {
			...
			Cmd("sysctl -w net.core.netdev_max_backlog=65535", true, true)
			...
			Cmd("tc qdisc replace dev "+mainNetworkInterface+" root fq_codel", true, true)
			Cmd("ip link set "+mainNetworkInterface+" txqueuelen 15000", true, true)
